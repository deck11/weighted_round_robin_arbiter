...
// ==============================================================
`timescale 1ns/1ps
module weighted_rr_control_s_axi
#(parameter
...
localparam
    ...
    ADDR_IER           = 6'h08,
    ADDR_ISR           = 6'h0c,
    ADDR_REQ_DATA_0    = 6'h10,
    ADDR_REQ_DATA_1    = 6'h14,
    ADDR_REQ_CTRL      = 6'h18,
    ADDR_WEIGHT_DATA_0 = 6'h1c,
    ADDR_WEIGHT_DATA_1 = 6'h20,
    ADDR_WEIGHT_CTRL   = 6'h24,
    ADDR_GRANT_DATA_0  = 6'h28,
    ADDR_GRANT_DATA_1  = 6'h2c,
    ADDR_GRANT_CTRL    = 6'h30,
    ADDR_RESET_DATA_0  = 6'h34,
    ADDR_RESET_CTRL    = 6'h38,
    WRIDLE             = 2'd0,
    WRDATA             = 2'd1,
    WRRESP             = 2'd2,
    WRRESET            = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_REQ_DATA_0: begin
                    rdata <= int_req[31:0];
                end
                ADDR_REQ_DATA_1: begin
                    rdata <= int_req[63:32];
                end
                ...
                ADDR_GRANT_DATA_0: begin
                    rdata <= int_grant[31:0];
                end
                ADDR_GRANT_DATA_1: begin
                    rdata <= int_grant[63:32];
                end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_REQ_DATA_0)
            int_req[31:0] <= (WDATA[31:0] & wmask) | (int_req[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_REQ_DATA_1)
            int_req[63:32] <= (WDATA[31:0] & wmask) | (int_req[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_GRANT_DATA_0)
            int_grant[31:0] <= (WDATA[31:0] & wmask) | (int_grant[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_GRANT_DATA_1)
            int_grant[63:32] <= (WDATA[31:0] & wmask) | (int_grant[63:32] & ~wmask);
    end
end
